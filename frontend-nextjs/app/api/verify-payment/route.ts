 ...
 *     on Vercel) and point Tesseract there.  Avoids any network call and is
 *     fast on every invocation after the first copy.
 ...
 *
 * NOTE: workerPath and corePath must NEVER be set to URLs — Node.js Worker
 * threads require a local filesystem path. Only langPath (the .traineddata
 * download location) accepts a URL as a fallback.
 */
...
export async function POST(request: NextRequest) {
  try {
    ...
    // to /tmp on cold start — no network round-trip needed on Vercel.
    let worker: Worker;
    try {
