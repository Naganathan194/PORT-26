...
const CoordinatorsSection: React.FC = () => {
  ...
  return (
    <section className={`py-32 ${theme === 'light' ? 'bg-gradient-to-b from-slate-50 via-slate-100 to-slate-50' : 'bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950'} relative overflow-hidden transition-colors duration-300`}>
      ...
      <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] ${theme === 'light' ? 'bg-indigo-500/3' : 'bg-indigo-600/5'} rounded-full blur-3xl pointer-events-none`} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {COORDINATORS.map((coordinator, idx) => (
            ...
            >
              ...
              {/* Card */}
              <div className={`relative ${theme === 'light' ? 'bg-gradient-to-br from-white/90 to-white/70 border-slate-200' : 'bg-gradient-to-br from-white/10 to-white/5 border-white/10'} backdrop-blur-sm border rounded-3xl p-8 h-full group-hover:border-white/20 transition-all duration-500`}>
                {/* Icon */}
                ...
                <div className="space-y-4">
                  <div>
                    <h3 className={`text-2xl font-bold ${colors.textPrimary} mb-2 transition-colors`}>{coordinator.name}</h3>
                  ...
                  <div className="space-y-3 pt-2">
                    <a href={`tel:${coordinator.contact}`} className={`flex items-center gap-4 ${colors.textTertiary} ${theme === 'light' ? 'hover:text-slate-900' : 'hover:text-white'} transition-colors group/link`}>
                      <div className={`w-10 h-10 ${theme === 'light' ? 'bg-slate-200 group-hover/link:bg-violet-200' : 'bg-white/5 group-hover/link:bg-violet-600/20'} rounded-xl flex items-center justify-center transition-colors`}>
                        <Phone className="w-4 h-4" />
                      </div>
                      <span className="text-sm">{coordinator.contact}</span>
                    </a>
                    <a href={`mailto:${coordinator.email}`} className={`flex items-center gap-4 ${colors.textTertiary} ${theme === 'light' ? 'hover:text-slate-900' : 'hover:text-white'} transition-colors group/link`}>
                      <div className={`w-10 h-10 ${theme === 'light' ? 'bg-slate-200 group-hover/link:bg-violet-200' : 'bg-white/5 group-hover/link:bg-violet-600/20'} rounded-xl flex items-center justify-center transition-colors`}>
